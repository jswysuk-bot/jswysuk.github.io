...
var main = (function($) { var _ = {
	...
	/**
	 * Build slides from current #thumbnails DOM (re-runnable after album swap).
	 */
	buildSlidesFromThumbnails: function() {

		// Clear slides state and remove slide elements from viewer.
			_.slides = [];
	...
	/**
	 * Rebuild viewer after #thumbnails HTML was replaced (e.g. album change).
	 */
	rebuildFromThumbnails: function() {

		if (_.locked)
			return;

		_.buildSlidesFromThumbnails();

		if (_.slides.length > 0 && !breakpoints.active('<=xsmall'))
			_.switchTo(0, true);
